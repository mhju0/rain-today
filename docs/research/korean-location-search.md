...
- [Verified] Live production checks on 2026-08-14 show that `강남구` resolves incorrectly to `강남구렁고개` in Buyeo, while `서울시` and `역삼` return no result. `역삼동` returns candidates with mixed Korean/English names. This establishes a correctness defect in the current country-wide search, not only a missing autocomplete interaction. [오늘비 location search](https://rain-today.vercel.app/api/locations/search?q=%EA%B0%95%EB%82%A8%EA%B5%AC)
